...
◆◆◆ ffmpeg 4.3以降でも動くように対策しました。(Thanks @ichinomoto for finding a workaroud)
使おうとしているffmpegのバージョンが 4.3以降であれば、 "http_seekable" を無効にするオプション (-http_seekable 0) を付加します。

~~◆◆◆ 2022年5月13日以降、サイトの仕様変更で、ffmpeg 4.3以降だと正常にダウンロード出来ません。当面はそれ以前のもの (推奨は 4.2.3 ～ 4.2.7)) を使うのが回避策になります。(注意事項 その5 に追加情報)~~
...
### その5
ffmpeg 4.3 以降で動かすための対策が見つかり入れました。ffmpeg 4.3以降に対して自動でオプションを追加します。
実行可能なffmpegを本スクリプトと同じディレクトリに置くと、そちらを優先して使うように変更しました。
また何かあったときに、逃げ道として使うことがあるかも (?)。
~~5/13問題（同様の問題が将来起きたときも) 対策で古めのffmpegを使うときに、システム全体の環境に影響を与えないためです。~~
~~エラー回避のためには、ffmpegの4.2系列のうちで、新しめのもの (4.2.3 以降) を推奨します。~~
~~ffmpegのバイナリは、 https://trac.ffmpeg.org/wiki/CompilationGuide/ あたりを参考に自分でビルドする（推奨）か、お使いの環境用にビルドされたものを探すかしてください。x264/265, libvpx, libops 等使わないコーデックのライブラリは disable してしまって大丈夫です。(使うのは aac, mp3lame, gnutls ぐらい)~~
Windows版には私のビルドした4.2.7の実行ファイルを添付しています。(少しの間残しておきます)

## 履歴

### V2.6.1
- ffmpeg 4.3以降でも正常にダウンロードするための対策を追加。(使おうとしているffmpegのバージョンが 4.3以降であれば、 "http_seekable" を無効にするオプション (-http_seekable 0) を付加します)
