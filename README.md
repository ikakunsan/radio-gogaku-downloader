...
(NEW) Windowsでは、Pythonのインストールは不要です。-sオプションは動作しません。（詳細は後述）
...
- ffmpeg.exeをパスの通ったところ、もしくは作業フォルダ (radio-gogaku-downloader.exeと同じフォルダ) に置きます。
  - https://www.gyan.dev/ffmpeg/builds/packages/ にある、
    ffmpeg-4.3.2-2021-02-27-full_build.7z で動作確認しています。このファイルをダウンロードして、7zip等で展開してください。
...
- 具体的な方法は、windowsディレクトリのREADME.txtをご参照ください。
