...
◆◆◆ (V2.5) サイトからのダウンロードは複数のセグメントに分けて行うのですが、たまに一部のセグメントでエラー (404) が発生してデータが欠損する事がありました。V2.5でその対策を入れました。欠損があったと判断した場合は最大５回までリトライします。(回数を変えたいときは RETRY_MAX の数字を書き換えてください)
