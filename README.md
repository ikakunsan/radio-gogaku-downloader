...
### その4
Windows上での動作について。
- PowerShellもしくはコマンドプロンプトから動かせます。バッチファイルを作るといいと思います。
- 事前にPython 3およびライブラリ等をインストールする必要があります。（他環境と同じ）
- ffmpeg.exeをパスの通ったところ、もしくはradio-gogaku-downloader.pyと同じフォルダに置きます。
  - https://www.gyan.dev/ffmpeg/builds/packages/ にある、
    ffmpeg-4.3.2-2021-02-27-full_build.7z で動作確認しています。
- ライブラリの関係で、PowerShell, コマンドプロンプト上では、
  ...
  1. 番組一覧から、不要な番組を含む行を行単位で削除。（不要の番組以外の行は残すこと）
  1. ファイルを保存。
  1. コマンドラインから実行。なおパスの区切り文字は / と \ のいずれでも可。

## 履歴
### V2.2.0
- Windowsの限定サポート。courses-***.jsonをマニュアルで編集しやすいように少し変更。
