...
◆◆◆ (V2.5) 64kbpsは音質が悪いので128kbpsをデフォルトにしました。

◆◆◆ (V2.5) データ欠損対策。サイトからのダウンロードは複数のセグメントに分けて行うのですが、たまに一部のセグメントでエラー (404) が発生してデータが欠損する事がありました。V2.5でその対策を入れました。欠損があったと判断した場合は最大５回までリトライします。(回数を変えたいときは RETRY_MAX の数字を書き換えてください)
...
- 音質 (-q オプション) のデフォルトを 128kbps (-q 1) に変更
